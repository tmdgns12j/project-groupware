...
<!-- left box -->
      <div class="mail1-detail">    
         ...
         <li class="nav-item">
            <a class="nav-link active" data-toggle="tab" href="#home">보낸메일함</a></li>
         </ul>      
   ...
   <div class="mail-send"><h1>보낸메일함</h1><br>
      <div class="letterwrap">
         <div class="letter">
         <form method="post" class="letter" name="mail">
         <input type="button" value="메일쓰기" class="button"
         onclick="window.open('<%=request.getContextPath()%>/mail/mailForm','window_name','width=700,height=500,location=no,status=no,scrollbars=yes');"> 
         
            <input type="submit" value="전체숨김" onclick="location.href='maildelete'">
            <input type="submit" value="숨김" onclick="mail.action='delete';">
            </div>
            <br> 
            <div class="table1">
            <table class="w3-table-all table table-hover">
               ...
               <c:forEach var="l" items="${list}">
                     <tr><!-- emi_tb -->
                     <td><input type="checkbox" name="RowCheck" value="${l.num}" onclick="checkSelectAll()"/></td>
                     <td>${num}  </td>
                     <c:set var="num"  value="${num-1 }"/> <!-- 편지번호 내림차순 -->
                     <td>${l.naverid} </td>
                     <td>${l.recipient}</td> <!-- 보낸사원번호 -->
                     <td>${l.title}</td> <!-- 받은사원번호 -->
                     <td>${l.contents}</td> <!-- 내용 -->
   ...
   <div class="mail-send"><h1>보낸메일함</h1><br>
      <div class="letterwrap">
         ...
         onclick="window.open('<%=request.getContextPath()%>/mail/mailwriteForm','window_name','width=430,height=500,location=no,status=no,scrollbars=yes');"> 
            ...
            <input type="submit" value="전체숨김" onclick="location.href='maildelete'">
            <input type="submit" value="숨김" onclick="mail1.action='delete1';">
            </div>
            <br> 
            <div class="table1">
            <table class="table table-hover" >
               ...
               <c:forEach var="l" items="${list}">
                  ...
                  <c:if test="${l.hidden1 != 'x' }">
                     ...
                     <td><a href="info?num=${l.MAIL_NUM}" >${l.MAIL_NUM}-go</a></td><!-- num은 게시물 번호 -->
                     <td>${l.hidden }</td>
                     <td>${l.hidden1 }</td>
