...
public class DateData {
   ...
   public Map<String, Integer> today_info(DateData dateData) {
      ...
      int startDay = cal.getMinimum(java.util.Calendar.DATE);//시작날
      int endDay = cal.getActualMaximum(java.util.Calendar.DAY_OF_MONTH);//월말날짜
      ...
      if (today_year == search_year && today_month == search_month) {
         SimpleDateFormat dsdf = new SimpleDateFormat("dd");//포맷지정
   ...
   private Map<String, Integer> before_after_calendar(int search_year, int search_month){
      ...
      if(before_month<0){
         before_month=11;//0=1월 11=12월 1->12로 넘어가는부분
         before_year=search_year-1;
      }
      
      if(after_month>11){//0=1월 11=12월 12->1로 넘어가는부분
         after_month=0;
